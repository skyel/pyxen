...
class Square:
    def __init__(self, board, pos):
        self.board = board
        self.pos = pos
        self.timestamp = TIMESTAMP.get()
        self.removestamp = TIMESTAMP.get()
        self.zobrist_strings = []
        for i in xrange(3):
            self.zobrist_strings.append(int(randy.random() * MAXINT))
...
#                self.board.white_dead += 1
        for neighbour in self.neighbours:
            if neighbour.color != EMPTY and neighbour.removestamp != TIMESTAMP.get():
...
class ZobristHash:
    def __init__(self, board):
        ...
        for square in self.board.squares:
            self.hash = self.hash ^ square.zobrist_strings[EMPTY]
        self.hash_set.append(self.hash)

    def update(self, square, color):
        self.hash = self.hash ^ square.zobrist_strings[square.color]
        self.hash = self.hash ^ square.zobrist_strings[color]

    def exists(self, _hash):
        for var in self.hash_set:
            if var == _hash:
                return True
        return False

    def add(self):
        if self.exists(self.hash):
            return
        self.hash_set.append(self.hash)

    def dupe(self):
		return self.exists(self.hash)
...
class Board:
    ...
    def useful(self, pos): 
        ...
        for neighbour in square.neighbours:
            ...
            if neighbour_ref.timestamp != TIMESTAMP.get():
                if neighcolor == self.color:  
                    neighs += 1
                else: 
                    opps += 1
                neighbour_ref.timestamp = TIMESTAMP.get()
...
class UCTNode:
    ...
    def select(self, board):
        ...
        if self.unexplored:
            i = int(randy.random() * len(self.unexplored))
